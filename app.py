...
def detail(keyword):
    api_key = '6b4a8c20-226c-4eec-bd21-b81eeb068e76'
    url = f'https://www.dictionaryapi.com/api/v3/references/collegiate/json/{keyword}?key={api_key}'
    response = requests.get(url)
    definitions = response.json()
    if not definitions:
        return render_template('error.html', keyword=keyword, msg=f'Word not found: {keyword}')

    if type(definitions[0]) is str:
        suggestions = definitions if isinstance(definitions[0], str) else []
